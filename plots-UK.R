...
theme_set(theme_bw())
png(file = paste("figures/","AR6_database mean welfares PLUS SD",".png",sep=""), width = 1200, height = 1200, units = "px") 
...
data_m<- data_m %>%
  filter(year %in% c(2030,2060,2100))%>%
  mutate(yearcat=paste0(year,Category))

png(file = paste("figures/","AR6_database mean welfares",".png",sep=""), width = 1200, height = 1200, units = "px") 
p=(ggplot(data_m, aes(x = yearcat, group = Category)) +
     geom_jitter(aes(y=Mean, color=Category), size=5) + 
     labs(title = paste("AR6-database:", "welfare metric means by rho and weight"),
          y = "welfare", x = "") +
     facet_grid( rho ~ weights, scales = "free_y", labeller=labeller(rho = rho.labs, weights = weight.labs))+
     scale_color_brewer(palette="BrBG")+
     scale_fill_brewer(palette="BrBG")+ theme(text = element_text(size = 28)))
